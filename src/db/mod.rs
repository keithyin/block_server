...
impl ServingInfoDb {
    ...
    pub async fn insert_new_file(&self, fpath: &str, tag: i32) -> anyhow::Result<()> {
        self.conn
            .execute(
...
mod test {
    ...
    #[tokio::test]
     async fn test_serving_file_database() {
        ...
        db.insert_new_file("/path/to/file1", 0).await.unwrap();
        db.insert_new_file("/path/to/file2", 0).await.unwrap();
        db.print_all_records().await.unwrap();
        println!("-------------------");
        println!("New arrived files:{:?}", db.get_new_arrived_files().await.unwrap());

        db.update_file_tag_for_reading("/path/to/file1").await.unwrap();
        db.print_all_records().await.unwrap();
        println!("-------------------");
        db.update_file_tag_for_deletion("/path/to/file2").await.unwrap();
        db.print_all_records().await.unwrap();
